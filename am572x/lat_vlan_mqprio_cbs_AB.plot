...
file_name = 'lat_vlan_mqprio_cbs_A.data'
file_name2 = 'lat_vlan_mqprio_cbs_B.data'
...
\nCompl: min = `sed -n '/min\ val/s/\(^.*\)= \(.\)/\2/p' "@file_name2" | sed -n '1p'`, \
max = `sed -n '/max\ val/s/\(^.*\)= \(.\)/\2/p' "@file_name2" | sed -n '1p'`, \
p-t-p = `sed -n '/peak-to-peak/s/\(^.*\)= \(.\)/\2/p' "@file_name2" | sed -n '1p'` \
mean+-RMS = `sed -n '/mean\ +-\ RMS/s/\(^.*\)= \(.\)/\2/p' "@file_name2" | sed -n '1p'` \
\nStack: min = `sed -n '/min\ val/s/\(^.*\)= \(.\)/\2/p' "@file_name2" | sed -n '2p'`, \
max = `sed -n '/max\ val/s/\(^.*\)= \(.\)/\2/p' "@file_name2" | sed -n '2p'`, \
p-t-p = `sed -n '/peak-to-peak/s/\(^.*\)= \(.\)/\2/p' "@file_name2" | sed -n '2p'` \
mean+-RMS = `sed -n '/mean\ +-\ RMS/s/\(^.*\)= \(.\)/\2/p' "@file_name2" | sed -n '2p'` \
\nVlan: min = `sed -n '/min\ val/s/\(^.*\)= \(.\)/\2/p' "@file_name2" | sed -n '3p'`, \
max = `sed -n '/max\ val/s/\(^.*\)= \(.\)/\2/p' "@file_name2" | sed -n '3p'`, \
p-t-p = `sed -n '/peak-to-peak/s/\(^.*\)= \(.\)/\2/p' "@file_name2" | sed -n '3p'` \
mean+-RMS = `sed -n '/mean\ +-\ RMS/s/\(^.*\)= \(.\)/\2/p' "@file_name2" | sed -n '3p'` \
\nDriver: min = `sed -n '/min\ val/s/\(^.*\)= \(.\)/\2/p' "@file_name2" | sed -n '4p'`, \
max = `sed -n '/max\ val/s/\(^.*\)= \(.\)/\2/p' "@file_name2" | sed -n '4p'`, \
p-t-p = `sed -n '/peak-to-peak/s/\(^.*\)= \(.\)/\2/p' "@file_name2" | sed -n '4p'`, \
mean+-RMS = `sed -n '/mean\ +-\ RMS/s/\(^.*\)= \(.\)/\2/p' "@file_name2" | sed -n '4p'`"

set terminal qt 4
plot[4000:4050] "< sed -e '0,/complete tx latency, us/ d' -e '/max/,$ d' -e '/----/ d' ". file_name2 \
with points pointtype 7 black title "complete tx latency (app -> wire), us", \
"< sed -e '0,/stack tx latency, us/ d' -e '/max/,$ d' -e '/----/ d' ". file_name2 \
with points pointtype 5 lc rgb "red" title "stack tx latency (app -> packet scheduler), us", \
"< sed -e '0,/between device (sched) tx latency, us/ d' -e '/max/,$ d' -e '/----/ d' ". file_name2 \
with points pointtype 3 lc rgb "blue" title "vlan layer tx latency (packet scheduler entrance -> driver entrance) , us", \
"< sed -e '0,/driver + packet scheduler tx latency, us/ d' -e '/max/,$ d' -e '/----/ d' ". file_name2 \
with points pointtype 1 lc rgb "green" title "driver tx latency (driver entrance -> wire) , us"

set terminal qt 5
plot[4950:5050] "< sed -e '0,/complete tx latency/ d' -e '/max/,$ d' -e '/----/ d' ". file_name2 \
with points pointtype 7 black title "complete tx latency (app -> wire), us", \
"< sed -e '0,/stack tx latency, us/ d' -e '/max/,$ d' -e '/----/ d' ". file_name2 \
with points pointtype 5 lc rgb "red" title "stack tx latency (app -> packet scheduler), us", \
"< sed -e '0,/between device (sched) tx latency, us/ d' -e '/max/,$ d' -e '/----/ d' ". file_name2 \
...
set terminal qt 6
plot[1:10000] "< sed -e '0,/complete tx latency/ d' -e '/max/,$ d' -e '/----/ d' ". file_name2 \
with points pointtype 7 black title "complete tx latency (app -> wire), us", \
"< sed -e '0,/stack tx latency, us/ d' -e '/max/,$ d' -e '/----/ d' ". file_name2 \
with points pointtype 5 lc rgb "red" title "stack tx latency (app -> packet scheduler), us", \
"< sed -e '0,/between device (sched) tx latency, us/ d' -e '/max/,$ d' -e '/----/ d' ". file_name2 \
...
set terminal qt 7
plot[1:100000] "< sed -e '0,/complete tx latency/ d' -e '/max/,$ d' -e '/----/ d' ". file_name2 \
with points pointtype 7 black title "complete tx latency (app -> wire), us", \
"< sed -e '0,/stack tx latency, us/ d' -e '/max/,$ d' -e '/----/ d' ". file_name2 \
with points pointtype 5 lc rgb "red" title "stack tx latency (app -> packet scheduler), us", \
"< sed -e '0,/between device (sched) tx latency, us/ d' -e '/max/,$ d' -e '/----/ d' ". file_name2 \
